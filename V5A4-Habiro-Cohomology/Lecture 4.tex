...
\begin{definition}[Habiro Ring of $(R,\square)$]\label{def: Habiro ring of framing}\todo{PS writes this as a proposition, but we didn't formally define the Habiro ring of the framing before. So isn't this just a definition? The discussion of finite index stuff seems to indicate that $\Hcal_{(R,\square)}$ can similarly be defined as the limit over completions of cyclotomic polynomials?}
...
\end{example}
Returning to a discussion of Habiro cohomology of a smooth $\ZZ$-algebra with framing $\square:\spec(R)\to(\GG_{m})^{d}$, we recall that there are lifts of the automorphism $\gamma_{i}$ to $\Hcal_{(R,\square)}$\todo{Is this just using that any element of $\Hcal_{(R,\square)}$ is an element of some $R^{(m)}$ and using $\gamma_{i}^{(m)}$ of \Cref{def: root of unity algebra} and that this works because it is compatible with Frobenii?}. This produces a $\ZZ^{d}$-action on $\Hcal_{(R,\square)}$, and we can define Habiro-Hodge cohomology to be the group cohomology of the action of $\ZZ^{d}$ on $\Hcal_{(R,\square)}$. 
