...
\begin{figure}[H]\label{fig: Habiro cohomology}
    ...
    \end{center}
    \caption{The role of Habiro cohomology highlighted in blue. Compare Figure \ref{fig: cohomology theories for schemes}.}
\end{figure}
...
One issue that arises in trying to na\"{i}vely generalize Habiro cohomology to schemes of higher dimension is that the specialization of prismatic cohomology over the $q$-de Rham prism at $q=1$ recovers de Rham cohomology, but at other roots of unity recovers only Hodge cohomology -- this does not put all roots of unity on equal footing. But if the $q$-de Rham cohomology could be modified to be Hodge cohomology in an appropriate manner. This was shown by Meyer-Wagner in \cite{MeyerWagner}. 
...
\begin{theorem}[Scholze]\label{thm: analytic Habiro cohomology}\marginpar{Here ``ring stack'' and ``analytic'' are to be taken in the sense of condensed mathematics \cite{AnalyticStacks}.}
    There is an explicit ring stack over an analytic version of the Habiro ring yielding a full six-functor formalism.\marginpar{While multiplication is easy to define in this ring, addition is not. In particular, the instructor remarks that he spent a whole day computing what $1+1$ is in this ring.}
\end{theorem}
\begin{remark}
